    ...
    var processPayments = function () {
        ...
        async.waterfall([
            ...
            function (workers, rounds, addressAccount, callback) {
                ...
                shareLookups = [['hgetall', 'IDA:shares:round10805']];
                startRedisTimer();
...
module.exports = function (poolConfigs) {
    let logger = loggerFactory.getLogger('PaymentProcessing', 'system');
    logger.info('Payment processor worker started');

    async.filter(['ida'], function (coin, callback) {
        SetupForPool(poolConfigs, function (setupResults) {
            logger.debug('Processing processor initialized. Setup results %s', setupResults);
            callback(null, setupResults);
        });
    }, function (err, coins) {
        if (err) {
            logger.error('Error processing enabled pools in the config');
        } else {
            var poolOptions = poolConfigs;
            var processingConfig = poolOptions.paymentProcessing;
