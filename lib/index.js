...
module.exports = class Reward {
    constructor(poolConfig) {
        ...
        this.rewardController = new Reward(poolConfig);
    ...
    async logic (content) {
        ...
        switch (messageContent.action) {
            case 'isValidShare':
                await this.rewardController.isValidShare(messageContent);
                break;
            case 'invalidShares':
                await this.rewardController.invalidShares(messageContent);
                break;
            case 'isValidBlock':
                await this.rewardController.isValidBlock(messageContent);
                break;
            case 'invalidBlock':
                await this.rewardController.invalidBlock(messageContent);
                break;
            default:
                break;
        }
        // 标记消息处理完毕
        await this.mq.deleteMessage(content.Message.ReceiptHandle);
    }
};
